...
function aggregateStats({ clickstream }) {
    ...
    if(clickstream?.length) {
        ...
        sortedCountries = generateSortedList(countries).slice(0,5)
        sortedDestinations = generateSortedList(destinations).slice(0,5)
        sortedIps = generateSortedList(ips).slice(0,5)
    } else {
        sortedCountries = [];
        sortedDestinations = [];
        sortedIps = [];
