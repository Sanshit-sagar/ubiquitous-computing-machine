...
class QueryManager {
  ...
  getAllPosts() {
    return this.client.query(q.Paginate(q.Match(q.Index('all_posts')))); 
  }

  getPosts() {
    // const { author = null, ...opts } = options;options = {}
    return this.client.query(
        q.Map(q.Paginate(q.Match('posts_sort_by_published_at_desc'))),
    ...
    return this.client
      .query(
        q.Map(
          ...
          q.Lambda(['updated_at', 'ref'], q.Get(q.Var('blog_posts')))
        )
      )
      .then(res => flattenData(res));
  }
