...
class QueryManager {
    ...
    getAllPosts() {
        return this.client.query(q.Paginate(q.Match(q.Index('slugged_posts')))); 
    }
    ...
    getPosts(options = {}) {
        ...
        return this.client
            .query(
                q.Map(
                    q.Paginate(
                        q.Index('slugged_posts_sorted_by_published_at_desc')),
                ),
    ...
    getPost(id) {
        return this.client
        .query(q.Get(q.Ref(q.Collection('slugged_posts'), id)))
        .then(res => flattenData(res));
    }
    ...
    createPost({ title, content, author }) {
        return this.client
        .query(
            q.Create(q.Collection('slugged_posts'), {
            data: {
                title,
                content,
                author,
