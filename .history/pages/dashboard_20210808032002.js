    ...
    return (
        <>
            {dataset.map(function(datum, index) {
                let validTime = datum?.click?.timestamp?.length!==0 ? true : false
                let timestr = validTime ? formatTimestamp(datum.click.timestamp) : ''
                ...
                return (
                    <div key={index}>
                        <Flex css={{ fd: 'row', jc: 'flex-start',ai: 'center', gap: '$2' }}>
                            ...
                            <> {validTime ?
                                <Flex css={{ fd: 'column', jc: 'flex-start', ai: 'flex-start', gap: '$1' }}> 
                                    <Text>  {timestr.split[0]} </Text>
                                    <Text>  {timestr.split[1]} </Text>
                                </Flex>: 
                                <Text> N/A </Text>
                            }</>
                        </Flex>
