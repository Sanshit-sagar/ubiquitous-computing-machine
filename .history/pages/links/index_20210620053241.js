...
const LinkEntry = ({ link, user }) => {
    ...
    const handleLinkEntryAction = (entryPathname) => {
        const slugForEntry = entryPathname.substring(6)
        dispatch({
            type: 'navigate',
            payload: {
                route: `/links/${slugForEntry}`
            }
        });
        router.push(`/links/${slugForEntry}`)
        toast.success(`Opening links/${slugForEntry}`)
    }
