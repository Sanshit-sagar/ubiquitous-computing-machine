...
function useFilteredStatList(email, filter)  {
    const { data, err } = useSWR(validEmail(email, filter) ? `/api/stream/filtered/${email}?filter=${filter}` : null)
...
export default function LinksPage({ metadata, session, loading, email }) {
