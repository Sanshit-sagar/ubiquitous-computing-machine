...
const TabulatedLinks = ({ user }) => {
    ...
    if(loading) return <p> your links are loading... </p> 
    if(error) return <h2> {error.message} </h2>

    let sortedLinks = linkEntries.sort((a, b) => parseInt(JSON.parse(b[1]).timestamp) - parseInt(JSON.parse(a[1]).timestamp))
...
export default function Links({ user }) {
    ...
    }, [links, linksLoading, linksError, sortedLinks, reval]);
