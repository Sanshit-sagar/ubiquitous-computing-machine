...
const LinkEntry = ({ link }) => {
    ...
    return (
        <>
            <td> {JSON.parse(link[1]).slug} </td> 
            <td> {JSON.parse(link[1]).url.substring(0, 25)} </td> 
            <td> {JSON.parse(link[1]).user} </td> 
            <td> {JSON.parse(link[1]).timestamp} </td> 
            <td>
                { 
                    !data && !error ? <p> ... </p> 
                    : error ? <p> !!!! </p> 
                    ...
                    return (
                        <tr key={index}>
                            {session && !loading && JSON.parse(value[1]).user===session.user.email ?
                                ...
                                <LinkEntry link={value} /> : loading ? <p> loading... </p> : <p> error! </p> 
                            }
                         </tr>
                    );
                })}
