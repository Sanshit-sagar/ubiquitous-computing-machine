...
const LinkEntry = ({ email, index, cellsInRow, deleteConfirmed, setDeleteConfirmed, toggle, toggleInfoModal }) => {
    ...
    return (
        <TableRow className="w-full overflow-x-scroll">
            ...
            /> 
            <TableCell>
                {!cells.config ? '' :  cells.config.password && cells.config.password.length ? 
                    <Badge 
                        value={
                            <div className="inline-flex align-start justify-start">
                                <LockOpenIcon className="h-6 w-6 text-green-500" />
                                <span> {cells.config.password} </span>
                            </div>
                        }
                    /> : <Badge value={<LockOpenIcon className="h-6 w-6 text-green-500" />} /> 
                }
            ...
            <TableCell>
                {!cells.config ? '' : cells.config.seoTags && cells.config.seoTags.length ? `[${cells.config.seoTags.length}]` : <p> -- </p>}
            </TableCell>
            <TableCell>
                <Badge value={!cells.config ? '' : cells.config.blacklist ? cells.config.blacklist.length : 0} /> 
            </TableCell>
