...
const LinkRow = ({ link, user }) => {
    ...
    const slug = fields.url || ''
    const destination = fields.url || ''
    const timestamp = fields.timestamp || ''
    const ttl = fields.ttl || ''

    if(!slug.length || !slug.destination.length || !timestamp.length || !ttl.length) return;

    return (
        <TableRow>
            <TableCell>
                <div className="flex items-center text-sm">
                    <span className="font-semibold ml-2"> {fields.url} </span>
                </div>
            </TableCell>
            <TableCell>
