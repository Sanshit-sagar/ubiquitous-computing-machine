...
const TabulatedLinks = ({ uid, email, user }) => {
    ...
    let linkEntries = Object.entries(links).reverse()
    // let sortedLinks = linkEntries.sort((a, b) => String(JSON.parse(b[1]).timestamp).localeCompare(JSON.parse(a[1]).timestamp))
                        ...
                        return (
                            <TableRow key={index}>
                                {/* {uid && value && JSON.parse(value[1]).user===uid ? */}
                                    <LinkEntry link={value} user={user} /> 
                                    {/* : null */}
                                {/* } */}
                            </TableRow>
                        );
