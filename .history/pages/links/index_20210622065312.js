...
const useListOfSlugs = (uid) => { 
    ...
    const { data, error } = useSWR(email && email.length ? `/api/aliases/${email}` : null, fetcher);

    return {
        links: data ? data.userSlugs : null,
        loading: !data && !error,
        error
    };
}
...
const TabulatedLinks = ({ uid, user }) => {
    ...
    let linksOnPage = Object.entries(links).slice(cursor, cursor + pageSize)
