...
const TabulatedLinks = ({ uid, email, user }) => {
    ...
    let linkEntries = JSON.stringify(Object.entries(links))
    
    // let sortedLinks = linkEntries.sort((a, b) => String(a.timestamp).localeCompare(b.timestamp))
    // let linksOnPage = sortedLinks.slice(cursor, cursor + pageSize)

    return (
        <p> {JSON.stringify(linkEntries)} </p>
        // <TableContainer>
        //     <Table>
        //         <TableHeader>
