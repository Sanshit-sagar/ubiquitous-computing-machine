...
import { DragHandleDots1Icon, TriangleDownIcon } from '@radix-ui/react-icons'

function useClickstream(email)  {
    // const { data, err } = useSWR(email && email?.length ? `/api/stream/filtered/${email}?filter=${filter}` : null)
    const { data, error } = useSWR(email && email?.length ? `/api/clicks/${email}` : null);
            ...
            <Flex css={{ width: '100%', fd: 'row', jc:'space-between', ai: 'flex-start'}}>
                {!isIndex && <>
                    <Flex css={{ fd:'row', jc:'flex-start', ai: 'flex-start', mr: '$3' }}>
                        {/* <AccessibleIcon label={'Drag Column'}>
                            <DragHandleDots1Icon />
                        </AccessibleIcon> */}
                        <Text>{name}</Text>
                    </Flex>
...
const ClickstreamWrapper = () => {
    ...
    const [didUpdateCache, setDidUpdateCache] = React.useState(false)

    useEffect(() => {
        if(!didUpdateCache && !isLoading && !isError && clicks?.length) {
            updateCache(data);
            setDidUpdateCache(true);
        }
    }, [didUpdateCache]); 
    ...
    return (
        <> 
            <span className="text-white font-large"> {didUpdateCache.toString()} </Text>
            <StatisticTable 
                columns={columns} 
                data={data}
                fetchData={fetchData}
