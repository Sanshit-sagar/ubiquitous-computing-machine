...
function useAllUsersLinks(email)  {
    ...
    return {
        dataset: data ? data.links : [],
        isLoading: !data && !error,
        isError: error
    };
}

const useClicksOrLinks = () => {
    let dataset = {};
    let loading = false;
    let error = null;

    const [doFetch, setDoFetch] = useState(true); 
    const [dataset, setDataset] = useState('clicks')
    const [session, sessionLoading] = useSession()
    const email  = !sessionLoading && session?.user ? session.user.email : ''

    const toggleDataset = () => {
        setDataset((dataset==='clicks') ? 'links' : 'clicks');
        setDoFetch(true);
    }

    const { dataArr, loading, error } = dataset==='clicks' ? useClickstream(email) : useAllUsersLinks(email);

    if(!loading) setDoFetch(false); 
...
const ClickstreamWrapper = () => {
    const [filter, setFilter] = useState('links');
    const [session, sessionLoading] = useSession()
    const email  = !sessionLoading && session?.user ? session.user.email : ''

    const { clicks, isLoading, isError } = useClicksOrLinks(email)
