...
const getColumns = ({ filter }) => {
    ...
    return filter==='clicks' ? clicksColumns : linksColumns;
}
...
const ClickstreamWrapper = () => {
    const [prevFetch, setPrevFetch] = useState('clicks')
    const [doFetchClicks, setDoFetchClicks] = useState(true);
    const [doFetchLinks, setDoFetchLinks] = useState(false);

    const [session, sessionLoading] = useSession()
    const email  = !sessionLoading && session?.user ? session.user.email : ''

    const { clicks, clicksLoading, clicksError } = useClickstream(doFetchClicks, email)
    const { links, linksLoading, linksError } = useAllUsersLinks(doFetchLinks, email)
    ...
    const toggleDataset = () => {
        if(doFetchClicks) {
            alert('Changing dataset from clicks to links')
        } else {
            alert('Changing dataset back from links to  clicks');
        }
        // if(prevFetch==='clicks') {
        //     setPrevFetch('links');
        //     setDoFetchLinks(true);
        // } else {
        //     setPrevFetch('clicks')
        //     setDoFetchLinks(true);
        // }
    ...
    let isLoading = doFetchClicks ? clicksLoading : linksLoading;
    ...
    const fetchData = React.useCallback(({ pageSize, pageIndex }) => {
        ...
        if(!isLoading && !isError && ((doFetchClicks && clicks?.length) || (doFetchLinks && links?.length))) {
            if (fetchId === fetchIdRef.current) {
                const startRow = pageSize * pageIndex
                const endRow = startRow + pageSize

                if(doFetchLinks) {
                    setData(links.slice(startRow, endRow));
                    setPageCount(Math.ceil(links.length / pageSize));
                    setDoFetchLinks(false) // ?-> fetch only once?
                } else {
                    setData(clicks.slice(startRow, endRow));
                    setPageCount(Math.ceil(clicks.length / pageSize));
                    setDoFetchClicks(false) //? -> fetch only once?
                }
                setFormatting(false)
            }
        }
    }, [clicks, links, clicksLoading, linksLoading, clicksError, linksError, doFetchClicks, doFetchLinks]); 

    let cellsLoading = sessionLoading || formatting || isLoading
    const columns = getColumns({ prevFetch });
