...
import { TriangleUpIcon, TriangleDownIcon, CheckIcon } from '@radix-ui/react-icons'
        ...
        <Box css={{ height: '30px', padding: '$1' }}>
            <Flex css={{ width: '100%', fd: 'row', jc:'space-between', ai: 'flex-start'}}>
                {isFirstCol ? 
                    ...
                    <Checkbox defaultChecked={false} id={name} aria-label={`Select row ${name}`}>
                        <CheckboxIndicator>
                            <CheckIcon />
                        </CheckboxIndicator>
                    </Checkbox>
                ...
                <>
                    ...
                    <AccessibleIcon label={`Sort by ${name}`}>
                        <button onClick={() => applySort(index)}>
                            {
                                  sortApplied && sortRowId===index 
...
const getColumns = () => {
    ...
    const clicksColumns = useMemo(() => [
        { 
            Header: <StyledHeader name={'#'} index={0} />, 
            accessor: 'id',
