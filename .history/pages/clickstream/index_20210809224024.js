...
function useClickstream({ email, filter })  {
    if(!filter?.length) {
        filter = 'clicks'
    }
    const { data, error } = useSWR(email?.length ? `/api/${filter}/${email}` : null);
...
const ClickstreamWrapper = () => {
    ...
    const { clicks, isLoading, isError } = useClickstream({ email, filter })
    ...
    return (
        <Box css={{ border: 'thin solid', borderColor: '$hiContrast', br: '$1' }}>
            <button onClick={() => setFilter('links')}> YOYOYOYOYO </button>
            <AriaTable 
                columns={columns} 
                data={data}
                fetchData={fetchData}
