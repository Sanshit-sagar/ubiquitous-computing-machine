...
const StyledHeader = ({ name, isIndex }) => {

    return (
        <Box css={{ height: '30px', padding: '$1' }}>
            <Flex css={{ width: '100%', fd: 'row', jc:'space-between', ai: 'flex-start'}}>
                {!isIndex && <>
                    <Flex css={{ fd:'row', jc:'flex-start', ai: 'flex-start', mr: '$3' }}>
                        {/* TODO: Add drag handle once drag and drop functionality is done */}
                        <Text size='2'> 
                            {name}
                        </Text>
                    </Flex>
                    ...
                    <AccessibleIcon label={`Sort by ${name}`}>
                        <Button size="xsmall">
                            <TriangleDownIcon />
                        </Button>
                    </AccessibleIcon> 
...
const getColumns = (cellsLoading, datasetId) => {
    
    const clicksColumns = useMemo(() => [
        { 
            Header: <StyledHeader name={'#'} isIndex={true} />, 
            accessor: 'id',
    ...
    if(!datasetId) return linksColumns; 
    return parseInt(datasetId)===0 ? clicksColumns : linksColumns
}
...
const ClickstreamWrapper = () => {
    ...
    const { clicks, isLoading, isError } = useAllUsersLinks(email)
    
    const [datasetId, setDatasetId] = React.useState(validDatasets[0])
    const [data, setData] = React.useState([])
    const [loading, setLoading] = React.useState(false)
    const [pageCount, setPageCount] = React.useState(0)
    const fetchIdRef = React.useRef(0)

    const handleDatasetIdUpdate = (updatedId) => {
        let updatedIdStr = validDatasets[updatedId]
        setDatasetId(updatedIdStr);
    }
    ...
    const columns = getColumns(cellsLoading, datasetId);
    ...
    return (
        <StatisticTable 
            ...
            pageCount={pageCount}
            datasetId={datasetId}
            setDatasetId={handleDatasetIdUpdate}
        /> 
    )
}
