...
const Click = ({ value, index }) => {
    ...
    const fields =  {
        ...
        'geodata': value.request_headers ? `${value.request_headers.city || ' '} ${value.request_headers.country || ' '} - ${value.request_headers.postalCode || ' '} ${value.request_headers.region || ' '} ${value.request_headers.metroCode || ' '}` : null,
        // 'coordinates': `(${value.request_headers.latitude}, ${value.request_headers.longitude})`,
        // 'metroCode': value.request_headers.metroCode,
        'ip': value.request_headers ? value.request_headers.ip ? value.request_headers.ip.substring(0, 10) + "..." : null : null,
        'system': value.request_headers ? value.request_headers.system ? value.request_headers.system.substring(0, 25) + "..." : null : null,  
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                { clicks && clicks.length ?
                                    clicks.sort((a, b) => { b.timestamp - a.timestamp }).reverse().map((value, i) => {
                                        return (
                                            <tr key={i}>
                                               { value ? <Click value={value} index={i} />  : null } 
                                            </tr>
                                        );
                                })}
                            </tbody>
