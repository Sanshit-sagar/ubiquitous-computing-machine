...
const Click = ({ value, index }) => {
    ...
    const fields =  {
        ...
        'slug': {
            primaryText: value.get('slug').substring(8),
            secondaryText: value.get('destination'),
        },
        'ip': {
...
const ClickStreamList = ({ clicks }) => {
    ...
    const chronoClickStream = clicks ? new Map([...clicks]) : null;
    // clicks.sort((a, b) => { b.timestamp - a.timestamp }).reverse()
                    ...
                    <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                        ...
                        <table className="table-auto min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                { chronoClickStream && chronoClickStream.length && 
                                    chronoClickStream.map((value, i) => {
                                        return (
                                            <tr key={i}>
