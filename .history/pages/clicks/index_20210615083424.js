        ...
        <>
            ...
            { clicks && clicks.length && 
                <div>
                    { clicks.map((value, i) => {
                        return (
                            <div key={i} style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-start' }}>
                                <p>{value.id}  </p>
                                <p>{value.slug} </p>
                                <p>{value.owner} </p>
                                {value.request_headers && 
                                    <div style={{ backgroundColor: 'red',display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'flex-start' }}>
                                        {/* <p> {value.request_headers['host']} </p> */}
                                        {/* <p> {value.request_headers['ip']} </p> */}
                                        {/* <p> {value.request_headers['system'] ? value.request_headers['system'].substring(0, 20) : ''} </p> */}
