...
const ClickStreamCache = async () => {
    ...
    const endpoint = session && session?.user ? `${baseUrl}/api/${session.user.email}` : null;
    const response = endpoint ? await fetch(endpoint) : ''
        

    return (
        <> {session && session?.user ? 
            <>
                response ?  <p> {JSON.stringify(response)} </p> : <p> getting clicks... </p>
            </>
                : loading ? <p> ...loading </p> : <p> error! </p>}
        </>
        // <StackedLayout
        //     pageMeta={clickStreamMetadata}
        //     children={!loading && !error ?  <ClickStreamTable clicks={clicks} />  : 
