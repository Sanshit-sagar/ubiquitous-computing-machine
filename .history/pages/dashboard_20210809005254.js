...
const FreqTable = ({ freqs, table, loading }) => {
    if(loading) return <MiniTableSkeleton />

    return (
        <>
            { freqs.map(function(value, index) {
                return (
                    <div key={index} style={{ width: '100%' }}>
                        <Flex css={{ width: '100%', fd: 'row', jc: 'space-between', ai: 'center', gap: '$2' }}>
                            <Text> {value[0]} </Text>
...
const DataTableWrapper = ({ dataset, title, loading }) => {
    if(!dataset && !loading) return null;
            ...
            <Flex css={{  fd: 'column', jc: 'flex-start', ai: 'center', gap: '$1' }}>
                ...
                <Box css={{ width: '100%', padding: '$2', height: '175px', overflowY: 'scroll' }}> 
                    <Flex css={{  fd: 'column', jc: 'flex-start', ai: 'stretch', gap: '$1' }}>
                       {
                              title==='Recent Views' ? <RecentViews dataset={dataset} loading={loading} /> 
                            : title==="Most Views" ? <MostViews  dataset={dataset} title={title} loading={loading} />
                            : <FreqTable freqs={mostFrequent} title={title} loading={loading} />
                        }
                    </Flex>
                </Box>
            </Flex>
