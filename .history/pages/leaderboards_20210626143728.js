...
const LeaderboardTables = ({ email, time }) => {
    ...
    let topUserAgents = aggregateStats(clickstream).sortedUserAgents.slice(0, 5)

    return (
        <div className="container mx-auto bg-white text-black font-extralight mt-5 p-2 m-2">
            <p> {JSON.stringify(topDestinations)} </p>
            <p> {JSON.stringify(topCountries)} </p>
            <p> {JSON.stringify(topIps)} </p>
            <p> {JSON.stringify(topUserAgents)} </p>
        </div>
    )
}
