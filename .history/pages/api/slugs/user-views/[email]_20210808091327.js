    ...
    try {
        if(!email) {
            ...
            clickstreamRaw.map(function(value, idx) {
                ...
                incrementMap(destinations, click.destination);
                incrementMap(ips, click.visitor.ip);
                incrementMap(countries, click.geodata.country);
                incrementMap(engines, JSON.stringify(ua.engine)); 
                incrementMap(browsers, JSON.stringify(ua.browser));
                incrementMap(operatingSystems, JSON.stringify(ua.os));
            });
            ...
            res.status(200).json({ 
                ...
                broswers: generateSortedList(browsers),
