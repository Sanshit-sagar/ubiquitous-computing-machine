...
export default async function handler(req, res) {
    try {
        ...
        if(!email) {
            ...
            .then((response) => {
                res.status(200).json({ views, formattedClicks: response, slugsByPopularity: maxViews });
            }).catch((error) => {
                res.status(200).json({ views, formattedClicks: clickstream, slugsByPopularity: maxViews }); 
            });
        }
    } catch (error) {
