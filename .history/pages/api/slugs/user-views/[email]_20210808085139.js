    ...
    try {
        if(!email) {
            ...
            clickstreamRaw.map(function(value, idx) {
                ...
                if(destinations && destinations[click.destination]) {
                    destinations[click.destination] += 1
                } else {
                    destinations[click.destination] = 1; 
                }
            });
            res.status(200).json({ ...views, ...clickstream, slugsByPopularity: maxViews, destinations: generateSortedList(destinations) })
        }
    } catch (error) {
...
export function generateSortedList(column) {
    return Object.entries(column).sort((a, b) => column[b[0]] - column[a[0]])[0]
}
