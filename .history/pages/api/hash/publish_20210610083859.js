...
export default async function create(req, res) {
    ...
    try {    
        ...
        const hashDetailsKey = `hash_content`
        const userHashKey = `user::${email}::valid_hashes`
        const userHashDetailsKey = `user::${email}::hashInfo::${hash}`
        ...
        pipeline.hset(`${userHashDetailsKey}`, serializedContent);
        pipeline.hincrby(`${userNumHashesKey}`, 1);
        pipeline.hincrby(`${globalNumHashesKey}`, 1);
        pipeline.hincrby(`${viewsKey}`, 1);
