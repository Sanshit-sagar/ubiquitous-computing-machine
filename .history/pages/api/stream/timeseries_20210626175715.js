    ...
    try {
        ...
        clickstream.forEach((click) => {
            ...
            if(details.timestamp || details.misc && details.misc?.finalTimestamp) {
                ...
                let hourstr = `${date.getFullYear()}-${date.getMonth()}-${date.getDate()}-${date.getHour()}`
                ...
                let currHourlyFreq = 1
                if(hourlyFreqs[hourstr]) {
                    currHourlyFreq = hourlyFreqs[hourstr] + 1
                }
                hourlyFreqs[hourstr] = currHourlyFreq
