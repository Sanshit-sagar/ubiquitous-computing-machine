    ...
    try {
        ...
        clickstream.forEach((click) => {
            ...
            if(details.timestamp) {
                ...
                windowEnd = (windowEnd===0) ? parseInt(details.timestamp) : Math.max(windowEnd, parseInt(details.timestamp))
                ...
                timeseriesDetails.push(JSON.stringify({
        ...
        res.status(200).json({ timeseries, timeseriesDetails, freqs, statistics }); 
    } catch (error) {
        res.status(500).json({ error: `Iteration ended at counter: ${count} due to error ${error.message}` })
    }
}