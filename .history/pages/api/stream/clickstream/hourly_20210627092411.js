...
function mergeSeries(start, end, freqs, stepSize) {
    ...
    console.log(`Generate series: ${mergedSeries}`);
    return mergedSeries;
}
...
export default async function handler(req, res) {
    ...
    try {
        ...
        clickstream.forEach((click) => {
            ...
            if(details.timestamp || details.misc && details.misc?.finalTimestamp) {
                ...
                if(filter==='hourly') {
                    ...
                    hourlyStr = 'let datestr = `${date.getFullYear()}-${date.getMonth()}-${date.getDate()}`'
                }
        ...
        console.log(`Cummulative: ${JSON.stringify(cummulative)}`);
        console.log(`Frequencies: ${JSON.stringify(freqs)}`); 
