...
function mergeSeries(start, end, freqs, stepSize) {
    ...
    for(i=0; i <= (end-start)/stepSize; i++) {
        ...
        mergedSeries.push({ x: datestr, y: parseInt(counter) });
    }
    console.log(`Generate series with ${mergedSeries.length} slots`);
    return mergedSeries;
}
...
export default async function handler(req, res) {
    ...
    if(!session) {
        res.status(500).json({ error: 'Unauthenticated request' });
    } 
    ...
    try {
        ...
        let cummulativeSeries = mergeSeries(minTime, maxTime, freqs, 60*60*1000)

        console.log(timeseries)
        console.log(cummulativeSeries)
        console.log(freqs)

        res.status(200).json({ timeseries, cummulativeSeries, freqs, skipped, count, average, max, minTime, maxTime, timespan })
    } catch (error) {
