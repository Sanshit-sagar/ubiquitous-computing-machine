...
export default async function handler(req, res) {
    ...
    try {
        ...
        let mergedSeries = mergeSeries(minTime, maxTime, freqs, 60*60*1000)

        console.log(timeseries)
        console.log(cummulativeSeries)
        console.log(freqs)

        res.status(200).json({ timeseries, mergedSeries, freqs, skipped, count, average, max, minTime, maxTime, timespan })
    } catch (error) {
