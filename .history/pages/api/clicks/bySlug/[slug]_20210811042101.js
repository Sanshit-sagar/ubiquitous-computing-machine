...
async function handler(req, res) {
    ...
    if(req.method !== 'GET') {
        ...
        .then((response) => {
            ...
            let { now, interval, start, end } = createWeeklyInterval(minDate, Date.now(), maxDate)

            res.status(200).json({ clicks, labels, freqs, total, now, interval, minDate: start, maxDate: end });
        }).catch((error) => { 
