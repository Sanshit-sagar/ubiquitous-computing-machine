...
function createLabelsAndFreqs(clicks) {
    ...
    if(clicks?.length) {
        clicks.map(function (value, index) {
            ...
            minTimestamp = Math.min(minTimestamp, parseInt(value.timestamp));
    ...
    return { labels, freqs, total, start: minTimestamp }; 
}
...
export default async function handler(req, res) {
    ...
    if(req.method !== 'GET') {
        ...
        .then((response) => {
            ...
            let { labels, freqs, total, start } = createLabelsAndFreqs(clicks)
            let { now, interval } = createWeeklyInterval(start)

            res.status(200).json({ clicks, labels, freqs, interval, total, now });
        }).catch((error) => { 
