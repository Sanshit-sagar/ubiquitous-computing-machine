...
function createWeeklyInterval(start, now, end) {
    ...
    return { now, interval: pastWeek, minDate: start, maxDate: end }
}
...
async function handler(req, res) {
    ...
    if(req.method !== 'GET') {
        ...
        .then((response) => {
            ...
            minDate = new DateTime(minDate).toLocaleString();
            maxDate = new DateTime(maxDate).toLocaleString();

            res.status(200).json({ clicks, labels, freqs, total, now, interval, minDate, maxDate });
        }).catch((error) => { 
