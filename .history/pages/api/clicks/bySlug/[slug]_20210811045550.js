...
async function handler(req, res) {
    ...
    if(req.method !== 'GET') {
        ...
        .then((response) => {
            ...
            minDate = Math.min(minDate, new Date(interval[0]).getTime());
            minDate = DateTime.fromMillis(parseInt(minDate)).toLocaleString();
            
            maxDate = Math.max(maxDate, new Date(interval[interval.length - 1]).getTime());
            maxDate = DateTime.fromMillis(parseInt(maxDate)).toLocaleString();

            interval.map(function(e, i) {
                if(!freqs[e]) {
                    freqs[e] = 0; 
                }
            });
