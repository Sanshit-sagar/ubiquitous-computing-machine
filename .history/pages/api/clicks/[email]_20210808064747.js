...
function formatAndSortClicks(clicks) {
    ...
    console.log("$$ Done Getting Clicks");
...
export default async function handler(req, res) {
    ...
    if(req.method !== 'GET') {
        ...
        if(!email || email.length < 5) {
            ...
            await axios.get(fetchUrl).then(async function(resp, idx) {
                data = formatAndSortClicks(response.data.clicks);
                numCached = await updateCache(data)
                
                console.log(`Added ${numCached} to the cache, Exiting...`)
                res.status(200).json({ 'clicks': data, numCached })
            }).catch((error) => { 
