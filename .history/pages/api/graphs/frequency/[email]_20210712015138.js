...
function getSortedFreqs(clicks) {
    ...
    while(idx < endIdx) {
        ...
        if(!freqs[dateKey]) {
            freqs[dateKey] = 0; 
            console.log(`Zeroing ${dateKey} in the map`);
        } 
    ...
    let freqsArr = [];
    Object.entries(freqs).map(function(value, index) {
        freqsArr.push({ 'x': value[0], 'y': value[1] });
    })
    freqsArr.sort((a,b) => parseInt(new Date(a.x).toString()) - parseInt(new Date(b.x).toString())); 
