...
const LeaderboardTables = ({ email, time }) => {
    ...
    return (
        <div className="container mx-auto bg-white  mt-4 p-2 w-5/6 h-5/6">
            <div className="flex-col flex-wrap justify-start align-center">
                {Object.entries(leaderboards).map(function(leaderboard, index) {
                    return (
                        <div key={index} className="flex-col justify-start align-stretch m-4 p-2 rounded-md shadow">
                            <LeaderboardTable data={leaderboard[1].data} />
                            {/* {JSON.stringify(leaderboard[1].data)} */}
                        </div>
                    )
                })}
            </div>
