...
function useClickstreamSummary()  {
    ...
    const { data: summary, error } = useSWR(email?.length ? `/api/slugs/user-views/${email}` : null)
    const { data: frequencies, error } = useSWR(summary ? [`/api/slugs/user-views/${email}`, summary.clickstream] : null)
