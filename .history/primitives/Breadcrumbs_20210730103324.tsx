...
let IReactElementProps: React.ReactChild | React.ReactFragment | React.ReactPortal

function Breadcrumbs(props) {
    let {navProps} = useBreadcrumbs(props);
    let children = React.Children.toArray(props.children);

    let child: IReactElementProps = null; 

    return (
        <nav {...navProps}>
            <ol style={{  id: 'row', jc: 'flex-start', ai: 'stretch', gap: '4$' }}>
                {children.map((child, i) => {
                    return (
                        <Text>
                            {JSON.stringify(React.cloneElement(children[i], {isCurrent: i === children.length - 1}))}
                        </Text>
                    );
                })}
            </ol>
