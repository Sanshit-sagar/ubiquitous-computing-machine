...
const CustomBreadcrumbs = () => {
    ...
    if(loading) return <ProgressBar css={{ marginTop: '$2' }} />
    
    return (
        <Flex css={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', paddingRight: '2px' }}>
            <StyledLink>
                <Flex css={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center'}}>
                    <span style={{ marginRight: '7.5px' }}>
                        <ProfileAvatar name={session.user.name} image={session.user.image} loading={loading} />
                    </span>
            ...
            <StyledLink css={{ fontSize: '14px', marginRight: '10px' }}>
                {   
                        state.currentPage.charAt(0)==='/' 
                    ? ` ${state.currentPage.charAt(1).toUpperCase()}${state.currentPage.substring(2)} ` 
                    : state.currentPage?.length ? ` ${state.currentPage.charAt(0).toUpperCase()}${state.currentPage.substring(1)} ` 
                    : 'N/A' 
                }
            </StyledLink>

            <div style={{ marginLeft: '5px', marginLeft: '5px', marginRight: '5px'}}> 
                / 
            </div>
            
            <StyledLink css={{ fontSize: '14px', marginLeft: '10px' }}>
                {state.currentTab?.length ? `${state.currentTab}` : ''}
            </StyledLink>
        </Flex>
    )
}
                ...
                {/* <>  
                    {
                        ...
                        <StyledButton>
                            <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center'}}>
                                <span style={{ marginTop: '2.5px', marginRight: '5px'}}>
                                    <StrikethroughIconSvg />
                                </span>
                                <span>
                                    {state.blacklist[0].substring(0,5)}...[{state.blacklist.length}]
                                </span>
                            </div>
                        </StyledButton> 
                        : 
