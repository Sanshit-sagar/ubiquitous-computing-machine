        ...
        <div className="inline-flex justify-start align-center">
            <div className="relative mr-10" ref={menuRef}>
                <div
                    ...
                    onClick={toggleMenu}
                >
                    {Object.entries(extraButtons).map(function(value, index, array) {
                        
                            return (
                                <div key={index} className="bg-gray-900 focus:bg-gray-600">
                                    <button 
                                        className="flex px-1 py-2 my-1 rounded-md ring-gray-300 hover:ring-gray-50 hover:ring-offset-2"
                                        onClick={() => {
                                            value[1].href.length ? 
                                            router.push(`${value[1].href}`) : 
                                            console.log('no destination')
                                        }}
                                    >
                                        <span className="text-md font-light text-gray-50">
                                            {value[1].name}
                                        </span>
                                    </button>
                                </div>
...
const NotificationButton = () => {
    ...
    return (
        <button 
            type="button"
            className="flex outline-yellow-600 p-1 rounded-md items-center justify-center text-white hover:bg-yellow-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-white-500"
        >
            <NotificationBell 
                className="h-6 w-6" 
                aria-hidden="true" 
...
function StackedLayout({ children, pageMeta }) {
    ...
    return (
    <div className="h-screen w-full bg-gray-900 flex overflow-x-scroll overflow-y-scroll">
        <div className="bg-gray-900 border-gray-50 overflow-x-scroll overflow-y-scroll md:block">
                        ...
                        {({ open }) => (
                            <> 
                                ...
                                >
                                    <Menu.Items
                                        static
                                        className="origin-top-right absolute right-0 mt-2 w-30 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none"
                                    >
