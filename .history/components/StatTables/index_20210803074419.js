...
const StatTables = ({ email }) => {
    ...
    const { clickstream, loading, error } = useUserClickstreams(email)
    const { sortedCountries, sortedIps, sortedDestinations, userAgents, skipped, count, loading, error, noData } = aggregateStats({ clickstream, loading, error })
    ...
    if(error || suaError) {
        return (
            <p> Error! `${ error ? error.message : suaError.message }` </p>
        )
    }
