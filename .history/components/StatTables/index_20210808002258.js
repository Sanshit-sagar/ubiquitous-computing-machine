...
const StatTables = () => {
    const { summary, email, loading, error } = useClickstreamSummary()

    if(loading) return <DashboardSkeleton /> 
    if(error) return <p> Error! {error.message} </p>

    let clickstream = summary?.clickstream ? aggregateStats(summary.clickstream) : []
        ...
        <Box css={{ width: '1275px', overflowX: 'scroll', bc: 'white'}}>
            <Flex css={{ width: '100%', fd: 'row', jc: 'space-between', ai: 'stretch', gap: '$2' }}>
                ...
                <Flex css={{ width:'1250px', minHeight: '500px', fd:'column', jc:'space-between', ai: 'stretch', mt: '$1' }}>    
                    <Text> {JSON.stringify(summary.views)} </Text>
                    <Text> {JSON.stringify(clickstream)} </Text>
                </Flex>
            </Flex>
        </Box>
    );
