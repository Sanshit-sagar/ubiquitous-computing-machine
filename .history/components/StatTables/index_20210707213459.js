...
import { Card,  Button, IconPieChart, IconDatabase, IconBarChart } from '@supabase/ui'
...
const ActiveDatasetSelector = ({ email }) => {
    ...
    return (
        <div className="w-72">
            <p> {JSON.stringify(state.activeDataset)} </p>
            <Listbox 
...
const useTimeseries = (email) => {
    ...
    return {
        periodStart: data ? data.earliestEvent : null,
        periodEnd: data ? data.latestEvent : null,
        maxViewsValue: data ? data.max : null, 
        maxViewsDate: data ? data.maxIndex : null,
        invalidDays: data ? data.skipped : null,
        totalDays: data ? data.count : null,
        avgDailyViews: data ? data.average : null,
        daysSortedByViews: data && data.freqs ? Object.entries(data.freqs).sort((a, b) => data.freqs[b[0]] - data.freqs[a[0]]) : null,
        loading: !data && !error,
        error: error
    };
}
...
const StatTables = ({ email }) => {
    ...
    const { sortedCountries, sortedIps, sortedDestinations, sortedUserAgents, userAgents } = aggregateStats(clickstream)
    const { sua, suaLoading, suaError } = useAggregatedUserAgentStats(email, Object.keys(userAgents))
    const { periodStart, periodEnd, maxViewsValue, maxViewsDate, invalidDays, totalDays, avgDailyViews, daysSortedByViews, loading, error }  = useTimeseries(email)

    const handleGraphTypeUpdate = (updatedType) => {
        setGraphType(updatedType)
        dispatch({
            type: 'assign',
            payload: {
                key: 'activeChart',
    ...
    const finalizedData = [
        { index: '0', title: 'Top Destinations', variable: 'destinations', stat: sortedDestinations, loading: loading },
        { index: '1', title: 'Top Dates', variable: 'Dates', stat: freqs || [], loading: loading || tsLoading },
        { index: '2', title: 'Top Countries', variable: 'country', stat: sortedCountries, loading: loading },
        { index: '3', title: 'Top IP Addresses', variable: 'ip', stat: sortedIps, loading: loading},
        { index: '4', title: 'Top Operating Systems', variable: 'Operating Systems', stat: sua.osNames, loading: loading || suaLoading },
        { index: '5', title: 'Top Browsers', variable: 'Web Browsers', stat:  sua.browsers, loading: loading || suaLoading },
        { index: '6', title: 'Top Engines', variable: 'Engines', stat: sua.engines, loading: loading || suaLoading },
    ];
            ...
            <div style={{ width: '1000px', height: '400px', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'stretch' }}>    
                <div style={{ maxWidth: '500px', minWidth: '500px', height: '400px', maxHeight: '400px', overflowY: 'scroll', padding: '5px', borderRadius: '5px' }}> 
                    <DataTable
                        ...
                        data={finalizedData[state.activeDataset.index].stat.splice(0, 5)}
                        width={finalizedData[state.activeDataset.index].width}
                        loading={finalizedData[state.activeDataset.index].loading}
                    />  
                </div>

                <div style={{ maxWidth: '425px', minWidth: '400px', height: '300px', padding: '5px', borderRadius: '5px' }}>
                    <DataVisualizer
                        data={finalizedData[state.activeDataset.index].stat.splice(0, 5)}
                        title={finalizedData[state.activeDataset.index].title}
