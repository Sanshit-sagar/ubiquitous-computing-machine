...
const ActiveDatasetSelector = ({ email }) => {
    ...
    const [neverSeen, setNervous] = useState()
    ...
    const { start, end, max, maxIndex, timeseries, freqs, average, userAgent: details , tsError }  = useTimeseries(email)

    useEffect(() => {
        if(neverSeen) {
            alert(`#1${JSON.stringify(sua)})`);
        } else {
            alert(`#2${JSON.stringify(sua)})`);
        }
    })
