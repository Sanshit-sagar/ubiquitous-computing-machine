...
const DataTable = ({ title, variable, data, loading }) => {
    ...
    useEffect(() => {
        if(maxSize!==state.datasetSize) {
            setMaxSize(state.datasetSize);
            
            if(maxSize <= localData.length) {
                let temp = data.splice(0, maxSize);
                setLocalData([...temp]);
                setMaxSize(localData.length); 
            } 
        }
    }, [maxSize, state.datasetSize]); 
