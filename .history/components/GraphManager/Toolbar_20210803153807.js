...
const ToolbarMenu = ({ selected }) => {
  ...
  return (
      <Box 
        css={{ 
          width: '100%',  py: '$1', px: '$2', height: '50px', bc: '#fff', color: 'transparent', 
          borderBottom: 'thin solid silver', br: '5px' 
        }}
      >
            ...
            {items.map(function(value, index) {
              return (
                <AccessibleStyledButton
                  ...
                  selected={selected}
                  handleClick={(e) => {
                    toast.success(`Changing tabs to ${value.id}, currently it is: ${state.selectedTabId}`)
                    dispatch({
                      type: 'select',
                      payload: {
                          key: 'selectedTabId',
