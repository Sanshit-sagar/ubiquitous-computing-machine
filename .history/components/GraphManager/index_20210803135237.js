...
export const LineChart = () => {
    ...
    return (
        <Line
            data={DataGenerator(state.cummFreqsArr, false, LINE_CHART_STR, state.freqsLabels, state.datasetStart, state.datasetEnd)}
            options={{
...
export const ScatterPlot = () => {
    ...
    return (
        <Scatter
            data={DataGenerator(state.scatterPlotArr, false, SCATTER_PLOT_STR, state.freqsLabels, state.datasetStart, state.datasetEnd)}
            options={{
                maintainAspectRatio: false,
                plugins: {
                    tooltip: {
...
export const BarChart = () => {
    ...
    return (
        <Bar
            data={DataGenerator(state.freqsArr, false, BAR_CHART_STR, state.freqsLabels, state.datasetStart, state.datasetEnd)}
            width={1200}
            height={475}
            options={{
                maintainAspectRatio: false,
        ...
        <Box css={{ height: '450px', width: '1200px', margin: '10px 5px' }}>
           {
            ...
            :   state.selectedTabId === 'chart' ? <LineChart />
            :   <ScatterPlot /> }
        </Box>
    )
}

function DataGenerator(freqsArr, doFill, graphName, freqsLabels, start, end) {
