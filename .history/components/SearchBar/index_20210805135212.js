...
const useUserClickstream = ({ doFetch, email }) => {
    if(!doFetch) return { clickstream: [], isLoading: false, isError: false, isWaiting: true }; 
    ...
    return {
        ...
        isError: error,
        isWaiting: !data && (email || !email?.length),
    };
}
...
const SearchBar = (props) => {
    ...
    const { clickstream, isLoading, isError, isWaiting } = useUserClickstream({ doFetch, email });

    useEffect(() => {
        if(isWaiting) return;
        if(!isLoading && !isError && clickstream && !searchArea.length && doFetch) {
            let temp = [];
    ...
    }, [clickstream, isLoading, isError, isWaiting, searchArea, doFetch]);
