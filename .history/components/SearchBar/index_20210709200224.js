        ...
        <div>
            <Input
                ...
                onChange={async (e) => {
                    ...
                    let tempResults = []; 
                    let fuseResponse = fuse.search(value)
                    fuseResponse.forEach(function(value, index) {
                        tempResults.push({
                            'slug' : fuseResponse.item.slug,
                            'score': fuseResponse.item.score
                        });
                    });
            ...
            <SearchResultsPopover 
                results={results && results.length ? results.length : '0'}
            /> 
        </div>
    )
}
