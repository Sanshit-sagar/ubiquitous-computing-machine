    ...
    return (
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 p-4 gap-4">
            {Object.entries(stats).map(function(val, index, array) {
                return (
                    <div key={index}>
                        <div class="bg-white hover:bg-black outline-black hover:outline-white text-black hover:text-white shadow-lg rounded-md flex items-stretch justify-between p-1 border-b-1 font-lightest group">
                            <span class="flex justify-start items-stretch w-10 h-10">
                                {array[index][1].icon}    
                            </span>
