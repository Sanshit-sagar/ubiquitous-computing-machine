...
export const GraphSkeleton = () => {
    ...
    return (
        <ContentLoader width={1280} height={500} viewBox="0 0 1280 500">
...
export const ScatterPlot = () => {
    ...
    return (
        // <div style={{  height: '475px', width: '1200px', margin: '10px 10px 20px 5px' }}>
            <Scatter
...
const ChartManager = ({ loading, data, error }) => {
    const state = useContext(GraphStore.State)

    if(loading || error) return <GraphSkeleton />

    if() return
    if(state.selectedTabId === 'chart') return <LineChart />
    return (
...
const DataCharts = ({ email }) => {
    ...
    return (
        <Flex css={{ fd: 'column', jc: 'flex-start', ai: 'stretch' }}>
            <GraphToolbar />
            <ChartManager loading={loading} data={data} error={error} />
        </Flex>   
    );
}
