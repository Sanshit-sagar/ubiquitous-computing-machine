...
function generateData(cummFreqArr, start, end) {
    let freqsLabels = [];
    cummFreqArr.forEach(function (value, index) {
...
const DataCharts = ({ email }) => {
    ...
    const [cummFreqArr, setCummFreqsArr] = useState([])
            ...
            <OptionsBar 
                barChart={
                    <div style={{ height: '100%', width: '100%', margin: '10px 5px 20px 5px' }}>
                        <Bar
                            data={generateData(cummFreqArr, data.start, data.end)}
                            width={1000}
                            height={500}
                            options={{
