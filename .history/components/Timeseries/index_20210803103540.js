...
export const LineChart = ({ cummFreqsArr, lineChartStr, data }) => {
    const state = useContext(GraphStore.State)

    return (
        <div style={{ height: '425px', width: '1200px', margin: '10px 10px 20px 5px' }}>
            <Line
                data={generateData(state.cummFreqsArr, true, state.lineChartStr, data.start, data.end)}
                options={{
...
export const ScatterPlot = () => {
    ...
    return (
        <div style={{  height: '425px', width: '1200px', margin: '10px 10px 20px 5px' }}>
            <Scatter
                data={generateData(state.scatterPlotArr, false, "Scattered", data.start, data.end)}
                options={{
                    maintainAspectRatio: false,
                    plugins: {
                        tooltip: {
...
export const BarChart = ({ freqsArr, barChartStr, data }) => {
    const state = useContext(GraphStore.State)

    return (
        <div style={{ height: '100%', width: '1200px', margin: '10px 10px 20px 5px' }}>
            <Bar
                data={generateData(state.freqsArr, false, "Barred", data.start, data.end)}
                width={1000}
                height={425}
                options={{
                    maintainAspectRatio: false,
...
const DataCharts = ({ email }) => {
    ...
    return (
        <Card interactive={true} ghost active={true}>
            <h1> {state.fetchCount}----{email} </h1>
            {/* <OptionsBar 
                bar={<BarChart  />}
                line={<LineChart />}
                scatter={<ScatterPlot  />}
            /> */}
            <h1> {JSON.stringify(state.cummFreqsArr)} </h1> 
        </Card>
    );
}
