...
export const ScatterPlot = ({ scatterPlotArr, scatterPlotStr, data }) => {
...
export const BarChart = ({ freqsArr, barChartStr, data }) => {
    ...
    const dataArray = state.freqsArr
    const dataStr = 'Views By Day'
    const data = 
...
const DataCharts = ({ email }) => {
    ...
    const dispatchArrUpdate  = (updatedArray) => {
        dispatch({
            type: `update_datasets`,
            payload: {
                key: ['freqsArr', 'cummFreqArr', 'scatterPlotArr'],
                value: updatedArray
            }
        });
    }
    ...
    return (
        <Card interactive={true} ghost active={true}>
            ...
            {/* <OptionsBar 
                ...
                scatter={<ScatterPlot  />}
            /> */}
            <h1> {JSON.stringify(state.cummFreqArr)} </h1> 
        </Card>
    );
}
