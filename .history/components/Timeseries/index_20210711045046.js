...
const DataCharts = ({ email }) => {
    ...
    const { cummulative, frequencies, loading, error } = useHourlyClickstream(email) 

    useEffect(() => {
        if(!graphData.length && cummulative && cummulative?.length) {
            setGraphData({cummulative});
    ...
    return (
        <Card>
            <div style={{ height: '500px', width: '500px', margin: '10px 5px 20px 5px' }}>
                <Line
                    data={generateData(graphData, data.statistics.timeseriesStart, data.statistics.timeseriesEnd)}
                    width={400}
                    height={400}
                    options={{
