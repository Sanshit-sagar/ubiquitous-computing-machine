...
const Greeting = () => {
  ...
  return  ( 
    <Flex css={{ fd: 'row', jc: 'space-between', ai: 'center', width: '100%' }}>
      ...
      <span className="text-sm font-extralight mt-3 mr-2">
        {`${session.user.name}`}
      </span> 
    </Flex>
  )
...
const AuthButton = ({ doLogin, doLogout }) => {
  ...
  return (
    <Button onClick={() => handleAuthRouting({ doLogin, doLogout })}>
      ...
      <Box css={{ my: '$1' }}>
        <Text> 
          {doLogin ? 'Login' : doLogout ? 'Sign Out' : 'Sign Up'}
        </Text>
      </Box>
    </Button>
  )
...
const CallToAction = () => {
  ...
  return (
      <Card interactive={true} ghost active={true} css={{ maxHeight: '525px' }}>
