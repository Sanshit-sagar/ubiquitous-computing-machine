...
const PageViews = ({ initialData }) => {
    ...
    const { data, error } = useSWR(`/api/hash/info/${initialData.id}`, fetcher)

    
    return (
        <>
        <h1> { initialData && initialData.id ? `ID: ${initialData.id}` : 'no slug found'} </h1> 
        <h1> {  !data && !error ? 'loading..' : `${JSON.stringify(data)}` } </h1> 
        </>
    );
}
