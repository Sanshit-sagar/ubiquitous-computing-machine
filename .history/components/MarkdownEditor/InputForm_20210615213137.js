...
const FormWrapper = () => {
    ...
    const mutateGlobalCache = ({ slug, url, ttl }) => {
        dispatch({
            type: 'publish_link',
            payload: {
                slug: slug,
                value: `https://analyticly.hashably.workers.dev/hashed/${slug}`,
                destination: url,
                config: {
                    ttl
                }
    ...
    return (
        <>
            ...
            <div className="bg-gray-50 px-4 py-4 sm:px-6">
                ...
                <subtitle> 
                    {state.newlyPublishedLink.length ? `Destination: ${state.newlyPublishedLink}` : null} 
                </subtitle>
            </div>
        </>
    )
