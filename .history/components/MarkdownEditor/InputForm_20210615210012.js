...
const ExpirySelector = ({ expiry, handleExpirationChange }) => {
...
const FormWrapper = () => {
    ...
    const publish = async ({ alias, link }) => {
        if(!alias || !alias.length || !link || !link.length) return;
        ...
        if(error) {
            ...
            setSlugWithPrefix(`https://analyticly.hashably.workers.dev/hashed/${alias}`);
        }
    }    
    ...
    const handleSubmit = () => {
        const alias = data ? `${data.slug}` : null
        const link = `${state.destinationUrl}`
        publish({ alias, link })
    }
