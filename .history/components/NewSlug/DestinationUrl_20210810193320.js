...
function formatOpenGraphData(data) {
    const { host, pathname, title, description, ogUrl,
            ogImage, ogImageAlt, ogImageWidth, ogImageHeight,
            ogSiteName, ogType, themeColor
    } = data
    return {
        title,
        host,
...
const OgOutput = () => {
    ...
    return <Text> {data && !error ? formatOpenGraphData(data) : 'N/A'} </Text>;
}
