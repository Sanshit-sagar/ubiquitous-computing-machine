...
const TagKey = ({ category, currentKey, handleKeyChange }) => {
  ...
  return (
      <Select
        ...
        icon={<IconActivity />}
      >
        {categoryKeyMap[category].map(function(value, index) {
          return (
            <Select.Option 
              key={index}
              value={categoryKeyMap[category][index]}
            > 
              {categoryKeyMap[category][index]}
            </Select.Option> 
          ); 
        })}
      </Select>
...
const TagManager = () => {
    ...
    const handleAddition = (toAppend) => {
      // setTagsList([...tagsList, `${category}:${currentKey}=${toAppend}`]);
      dispatch({
          type: 'append',
          payload: {
              key: 'seoTags',
