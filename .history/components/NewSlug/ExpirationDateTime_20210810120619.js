...
const isValidDate = (date) => Object.prototype.toString.call(date)==='[object Date]' && isFinite(date)
...
const ExpirationInput = () => {
    ...
    return (
        <> 
        <Text> --{expiry}--{isValid.toString()} </Text>
        <input 
            name="ttl" 
            id="ttl" 
            value={expiry} 
            ...
            type='datetime-local'
            className="mt-2 mb-3 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-85 sm:text-sm border-gray-300 rounded-md dark:text-white dark:bg-gray-700"
