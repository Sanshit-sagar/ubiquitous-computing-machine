...
function NewSlugNavMenu() {
    ...
    return (
        <Menu className='w-full h-full mr-2 text-gray-700 bg-white dark:bg-gray-700 dark:text-white'>
            {items.map((item, i) => {
                return (
                    <Menu.Group key={i} title="Actions">
                        {item.content.map(function(value, j) {
                            return (
                                <Menu.Item 
                                    index={j}
                                    icon={items[i].content[j].icon}
                                    showActiveBar
                                    active={state.currentTab===items[i].content[j].id}
                                    onClick={handleTabChange}
                                >
                                    {items[i].content[j].title}
                                </Menu.Item>
                            );
                        })} 
                    </Menu.Group>
