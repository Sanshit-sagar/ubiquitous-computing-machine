...
const DataChart = ({ data, title, type }) => {
    ...
    return (
        <span className="text-md font-extralight"> 
            { 
                type==='pie' ?
                <PieChart sortedData={data} title={title} type={type} /> :
                <BarChart sortedData={data} title={title} type={type} />
            }
        </span>
    )
}
...
const LeaderboardTable = ({ data, title, type }) => {
    ...
    return (
        <div className="w-auto bg-white text-black rounded-md shadow-lg">
            ...
            {displayType!=='table' && <p className="text-black"> {JSON.stringify(data)} </p>}
        </div>
    )
}
