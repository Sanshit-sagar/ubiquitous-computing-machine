...
import { CornersIcon, Cross2Icon, CheckIcon, Pencil1Icon, PlusIcon } from '@radix-ui/react-icons'
...
const ActionButton = ({ detail, index }) => {
    const handleAction = () => {
        if(detail?.length) {
    ...
    return (
        <AccessibleStyledButton
            label={'Edit'}
            icon={detail?.length ? (index!==1 && index!==2 ? <Pencil1Icon /> : null) : <PlusIcon /> }
            handleClick={handleAction}
            content={detail?.length ? 'Update' : 'Add'}
        />
    )
}
    ...
    return (
        <SlugField>
            ...
            <Flex css={{ fd: 'row', jc: 'flex-end', ai: 'center' }}>
                ...
                <ActionButton detail={detail} index={index} />
            </Flex>
        </SlugField>
    )
}
