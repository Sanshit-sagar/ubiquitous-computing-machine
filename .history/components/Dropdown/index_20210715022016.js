...
const DropdownMenu = () => {
  ...
  let label = <Button loading={loading} text={session && session?.user ? `${session.user.name}` : 'Login'} minimal={true} fill={true} alignText="right" rightIcon={session && session?.user ? <ProfileAvatar image={session.user.image} name={session.user.name} loading={loading} /> : null} />
