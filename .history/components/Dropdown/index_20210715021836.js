...
const DropdownMenu = () => {
  ...
  let label = <Button loading={loading} text={session && session?.user ? `${session.user.name}` : 'Login'} minimal={true} fill={true} alignText="right" rightIcon={<img src={session.user.image} alt={session.user.name} height={25} width={25} />} />
