...
const Greeting = () => {
  ...
  const uiState = useContext(NewSlugStore.State)

  const [session, loading] = useSession()

  if(!session || !session?.user || !session.user.name?.length) return <Loader />; 
  if(loading || (!loading && !session?.user || !loading && !session.user?.name)) return <p> error! </p>; 
