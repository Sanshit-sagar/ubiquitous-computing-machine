...
export const useSlugDetails = (slug) => {
    const { data, error } = useSWR(email && email.length && slug && slug.length ? `/api/slugs/${slug}` : null);
...
const SlugProfile = ({ slug }) => {
    ...
    if(error) return <span className="text-sm font-extralight"> error: {error.message} </span>
    ...
    return (
        <div className="flex-col justify-start align-stretch">
            ...
            <span className="text-sm font-extralight inline-flex justify-evenly align-start">
                {loading ? <Loader /> : details.url}
            </span>
            ...
            <div className="text-xs font-extralight">
                {   
                    ...
                    <>
                        ...
                        <div> {JSON.stringify(details.config)}</div>
                    </>
                }
            </div>
        </div>
    ...
    const handleCopy = () => {
        toast((t) => (
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'stretch' }}>
              ...
              <input placeholder="Paste it here" style={{ border: 'thin solid black', borderRadius: '5px', marginTop: '10px', padding: '2.5px' }} />
            </div>
        ));   
    }

    return (
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', minWidth: '260px', maxWidth: '260px' }}>
            <span>
                <Dialog>
                    <DialogTrigger>
