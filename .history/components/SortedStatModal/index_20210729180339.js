...
const SortedStatModalWrapper = ({ email, filter }) => {
    ...
    useEffect(() => {
        setLoading(true);
        if(email?.length && filter?.length) {
            fetch(`/api/stream/filtered/${email}?filter=${filter}`) 
                .then((response) => {
                    alert(`Retrieved ${JSON.stringify(response)}`)
                    setData(`${response.filteredData}`);
                    console.log(`Recieved: ${JSON.stringify(response)} at ${new Date().getTime().toLocaleString()}`);
                }).catch((error) => {
                    setError(`${error.message}`);
                    console.log(`Error: ${error.message} at ${new Date().getTime().toLocaleString()}`);
                });
        }
        setLoading(false);
    }, []);

    let cellsLoading = loading || (!data && !error)
    ...
    const fetchData = React.useCallback(({ pageSize, pageIndex }) => {
        ...
        if(!loading && !error && filteredData && filteredData.length) {
