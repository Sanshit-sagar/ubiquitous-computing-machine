...
const StatisticTable = ({ columns, data, fetchData, loading, pageCount: controlledPageCount }) => {
    ...
    return (       
        <Box css={{ border: 'thin solid transparent', bc: 'white',  width: '1275px', overflowX: 'scroll' }}>
            <Flex css={{ fd: 'column', jc: 'flex-start', ai: 'stretch' }}>
                ...
                { loading ? <TableSkeleton />  :  
                    ...
                    >
                            ...
                            {headerGroups.map(headerGroup => (
                                <tr {...headerGroup.getHeaderGroupProps()}>
                                    {headerGroup.headers.map(column => (
                                        <th {...column.getHeaderProps()} style={{ padding: '3.5px'}}>
                        ...
                        <tbody {...getTableBodyProps()} style={{ height: '400px', maxHeight: '400px', overflowY: 'scroll' }}>
                            {page.map((row, i) => {  
                                prepareRow(row);
