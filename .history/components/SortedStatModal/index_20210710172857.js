...
const SortedStatModal = ({ filter, email }) => {
    const [session, loading] = useSession()
    const email = session && session?.user ? session.user.email : ''

    // const { clickstream, isLoading, isError } = useUserClickstreams(email)
    // const { uniques, isLoading, isError } = useUserUniqueClickstream(email)
    const { sortedArray, isLoading, isError } = useUserClickstreams(email, filter)
    ...
    const fetchData = React.useCallback(({ pageSize, pageIndex }) => {
        ...
        setTimeout(() => {
            if (fetchId === fetchIdRef.current) {
                ...
                if(!isLoading && !isError && uniques && uniques?.length) {
                    setData(uniques.slice(startRow, endRow));
                    setPageCount(Math.ceil(uniques.length / pageSize));
                } else {
                    setData([]);
                    setPageCount(0)
                }
