...
function useUserUniqueClickstream(email) {
    const { data, error } = useSWR(email && email?.length ? [`/api/stream/users/${email}?time=${time}`, time] : null);
    return {
...
const SortedStatModal = () => {
    ...
    const { uniques, loading, error } = useUserUniqueClickstream(email)
    ...
    const fetchData = React.useCallback(({ pageSize, pageIndex }) => {
        ...
        setTimeout(() => {
            if (fetchId === fetchIdRef.current) {
                ...
                if(!loading && !error && uniques && uniques?.length) {
