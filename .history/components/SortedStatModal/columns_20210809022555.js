...
export const SlugViewsCell = ({ slug }) => {
    ...
    if(error || !views) return <p> error! </p>;

    return (
        <StyledCell>
            <Flex css={{ fd: 'column', jc:'flex-start', ai: 'center' }}>
            <span className="text-xs font-extralight">  {views.total} total </span>
            <span className="text-xs font-extralight">  {views.unique} unique </span> 
