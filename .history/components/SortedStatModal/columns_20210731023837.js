...
const StyledLifeLeftCell = ({ value, maxValue }) => {
    ...
    return (
        <StyledCell value={`${value}-${maxValue}`} />
    )
}
...
const getColumns = (key, email, cellsLoading) => {
    ...
    const allLinksColumns = useMemo(() => [
        ...
        },
        {
            ...
            Cell: ({ value }) => {
                let timestampVal = parseInt(value)
                let localeTimeStr = DateTime.fromMillis(timestampVal).toLocaleString(DateTime.DATETIME_SHORT_WITH_SECONDS)
        ...
        },
        {
            ...
            Cell: ({ value }) => {
                ...
                return (
                    <StyledCell value={doesExpire ? `${localeTimestamp}` : `N/A`} />
                );
            }
        },
        {
            ...
            Cell: ({ value }) => {
                ...
                return (
                    <StyledLifeLeftCell value={progress} maxValue={100} />
                );
            }
        },
        {
