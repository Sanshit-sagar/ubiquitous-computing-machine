...
const StyledSelectMenu = ({ loading, pageSize, setPageSize, pageIndex, pageOptions }) => {
    // const [open, setOpen] = React.useState(false);
    let items = [5, 6, 7, 8, 9, 10, 15, 20, 25, 30, 40, 50];

    return (
        <Box>
            <SelectRoot sideOffset={5}>
                <SelectTrigger>
                    <Flex css={{ fd: 'row', jc: 'space-between', ai: 'stretch', gap: '$2' }}>
                        <Text size='2' css={{ mt: '$1' }}> 
                            Showing {pageSize}
                        </Text>
                ...
                <SelectContent>
                    <SelectRadioGroup 
                        value={pageSize} 
                        onValueChange={(e) => {
                            toast.success(`Changing page size to ${pageSize}`)
                        }}
                    >
                        {items.map(numItems => (
                            <SelectRadioItem  
                    ...
                    return (
                        <div key={index}>
                            <ActionButtonItem 
                                ...
                                onClick={(e) => { toast.success(`Clicked ${e.currentTarget.value}`)}}
                            > 
                                <Text> {loading ? 'loading...' : action.category} </Text> 
...
const TableController = ({
    ...
    return (
        <div className={uiState.darkMode ? darkTheme : lightTheme}>
            <Box css={{ py: '$2', px: '$3', height: '50px', bc: '$loContrast', color: '$hiContrast' }}>
                <Flex css={{ fd: 'row', jc: 'space-between', ai: 'stretch' }}>
                    ...
                    <TableActionsGroup loading={loading} />
