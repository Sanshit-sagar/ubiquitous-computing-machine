...
function normalizeTimeseries(freqs) {
    ...
    let freqsSorted = Object.entries(freqs).sort((a, b) => a[1].x - b[1].x); 

    Object.entries(freqsSorted).forEach(entry => {
        runningTotal = entry[1] + runningTotal
        freqsArr.push({ 
            x: entry[0], 
            y: parseInt(runningTotal) 
        });
    })
    return 
}
...
function ClickstreamTimeseries() {
    ...
    return (
        <div className="mt-4 h-full w-full rounded-md shadow-lg bg-white hover:bg-blue-200">
            <OptionsBar /> 
            <LineChart  data={normalizeTimeseries(data.freqs)} />
