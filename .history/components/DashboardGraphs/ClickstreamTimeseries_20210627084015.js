...
function ClickstreamTimeseries() {
    ...
    const { data, error } = useSWR(`/api/stream/clickstream/hourly`, fetcher)

    if(!data && !error) return <Loader />
    if(error) return <p> Error! </p>

    return (
        <div className="w-full min-w-full mt-3 rounded-md shadow-lg">
            <OptionsBar selected={selected} handleSelect={handleSelect} /> 
        
            { 
                  selected==='hourly' ? <LineChart data={data.cummulative} title="Cummulative" />
                // : selected==='hourly' ? <LineChart data={data.timeseries} title="Timeseries" />
                : null 
            } 
